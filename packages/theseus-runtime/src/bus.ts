 ...
 */
import { Effect, Layer, PubSub, type Scope, ServiceMap, Stream } from "effect";
...
export class MessageBus extends ServiceMap.Service<
  MessageBus,
  {
    publish: (topic: string, from: string, payload: unknown) => Effect.Effect<void>;
     ...
     */
    subscribeAll: () => Effect.Effect<PubSub.Subscription<BusEnvelope>, never, Scope.Scope>;
  }
>()("MessageBus") {}
...
export const MessageBusLive = Layer.effect(MessageBus)(
  Effect.gen(function* () {
    ...
    return MessageBus.of({
      publish: (topic, from, payload) => PubSub.publish(ps, { topic, from, payload }),

      subscribe: (topic) => Stream.fromPubSub(ps).pipe(Stream.filter((e) => e.topic === topic)),
