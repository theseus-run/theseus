...
export const makeInMemoryCapsule = (slug: string): Effect.Effect<CapsuleRecord> =>
  Effect.gen(function* () {
...
export const InMemoryCapsuleStore: Layer.Layer<CapsuleStore> = Layer.succeed(CapsuleStore)({
  create: ({ slug }) => makeInMemoryCapsule(slug),
});

export const CapsuleLive = (slug: string): Layer.Layer<Capsule> =>
  Layer.effect(Capsule)(makeInMemoryCapsule(slug));
