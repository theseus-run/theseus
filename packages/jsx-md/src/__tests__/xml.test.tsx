...
import { H2, Li, P, render, Ul } from "../index.ts";
...
describe("XML tags", () => {
  ...
  test("string attributes", () => {
    expect(
      render(
        <task type="summary" priority="high">
          content
        </task>,
      ),
  ...
  test('escapes " and & in attribute values', () => {
    expect(render(<task type='a"b' label="x & y" />)).toBe(
      '<task type="a&quot;b" label="x &amp; y" />\n',
    );
  });
...
describe("object as XML attribute", () => {
  test("passing an object as attribute value throws with helpful message", () => {
    expect(() => render(<task data={{ key: "value" }} />)).toThrow(
      'jsx-md: attribute "data" received an object value',
    );
  });

  test("error message mentions JSON.stringify", () => {
    expect(() => render(<task data={{ key: "value" }} />)).toThrow("JSON.stringify");
  });
});
