...
import { createContext, render, useContext, withContext } from "../index.ts";
...
describe("createContext / useContext / withContext", () => {
  ...
  test("nested withContext — innermost value wins", () => {
    ...
    const result = withContext(ctx, 1, () => withContext(ctx, 2, () => String(useContext(ctx))));
...
describe("Context.Provider", () => {
  ...
  test("Provider restores default after render completes", () => {
    ...
    render(<ThemeCtx.Provider value="dark">{""}</ThemeCtx.Provider>);
  ...
  test("outer Provider value is restored after inner Provider exits", () => {
    ...
    expect(
      render(
        <ThemeCtx.Provider value="dark">
          <ThemeCtx.Provider value="high-contrast">{""}</ThemeCtx.Provider>
          <ShowTheme />
        </ThemeCtx.Provider>,
      ),
    ).toBe("dark");
  ...
  test("Provider renders children normally (passthrough when no consumer)", () => {
    ...
    expect(render(<Ctx.Provider value="y">{"hello"}</Ctx.Provider>)).toBe("hello");
  });
