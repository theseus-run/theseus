...
describe("headings", () => {
  ...
  test("H2 with mixed inline children", () => {
    expect(
      render(
        <H2>
          <Code>fn</Code> returns <Italic>value</Italic>
        </H2>,
      ),
    ).toBe("## `fn` returns *value*\n\n");
  });
...
describe("P", () => {
  ...
  test("mixed inline children", () => {
    expect(
      render(
        <P>
          <Bold>x</Bold> and <Italic>y</Italic>
        </P>,
      ),
    ).toBe("**x** and *y*\n\n");
  });
...
describe("Codeblock", () => {
  ...
  test("with lang — fence tagged", () => {
    expect(render(<Codeblock lang="ts">{"const x = 1"}</Codeblock>)).toBe(
      "```ts\nconst x = 1\n```\n\n",
    );
  });
    ...
    expect(
      render(
        <Blockquote>
          <Ul>
            <Li>
              <Strikethrough>old item</Strikethrough>
            </Li>
          </Ul>
        </Blockquote>,
      ),
